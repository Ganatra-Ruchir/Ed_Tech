...
export default function MessagesPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/messages?conversationId=${selectedId}`).then((response) => response.json()).then((data) => setMessages(data.messages ?? []));
  }, [selectedId]);
  ...
  const selectedName = useMemo(() => selected?.isGroup ? selected.title ?? "Group" : selected?.members.map((member) => member.user.name).join(", ") ?? "Conversation", [selected]);
  ...
  async function sendMessage(event: React.FormEvent) {
    event.preventDefault();
    if (!selectedId || (!body.trim() && !file)) return;
    const formData = new FormData(); formData.set("conversationId", selectedId); formData.set("body", body); if (file) formData.set("file", file);
    ...
    setMessages((current) => [...current, data.message]); setBody(""); setFile(null); if (fileRef.current) fileRef.current.value = ""; await loadConversations();
  }

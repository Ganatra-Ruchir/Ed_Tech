...
export default async function FacultyAssignmentsPage() {
  ...
  return (
    <div className="space-y-5">
      ...
      {assignments.length === 0 ? <Card className="p-5"><p className="text-sm text-zinc-500">No assignments created yet.</p></Card> : <div className="space-y-2">{assignments.map((assignment) => <Card key={assignment.id} className="flex items-start gap-3 p-4"><span className="flex h-9 w-9 items-center justify-center rounded-md bg-[#6b1029]/[0.08] text-[#6b1029]"><FileText size={16} /></span><div className="min-w-0 flex-1"><p className="font-semibold text-zinc-900">{assignment.title}</p><p className="text-xs text-zinc-500">{assignment.batch.name} · {assignment._count.submissions} submission(s){assignment.dueAt ? ` · Due ${new Date(assignment.dueAt).toLocaleDateString("en-IN")}` : ""}</p>{assignment.description && <p className="mt-1 text-sm text-zinc-600">{assignment.description}</p>}</div>{assignment.attachmentUrl && <a href={assignment.attachmentUrl} target="_blank" rel="noreferrer" className="text-xs font-medium text-[#6b1029] hover:underline">View PDF</a>}</Card>)}</div>}
    </div>
  );
}